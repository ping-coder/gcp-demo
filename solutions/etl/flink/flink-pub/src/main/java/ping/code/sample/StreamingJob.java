...
public class StreamingJob {
	...
	public static void main(String[] args) throws Exception {
		...
		// tencent code
		DataStream<String> ds = env.addSource(mySqlSource);
		DataStream<Tuple2<String, String>> transformedDs = ds.map(new MapFunction<String, Tuple2<String, String>>() {
			@Override
			public Tuple2<String, String> map(String value) throws Exception {
				return new Tuple2<>(value, value);
			}
		});
		transformedDs.addSink(new GcpSink("peace-demo", "meitu-pub"));
		ds.map(StreamingJob::printAndReturn);
//		ds.addSink(pubsubSink);
...
//				.addSink(pubsubSink);
	...
	private static String printAndReturn(String s) {
		System.out.println("Processed message with payload: " + s);
		return s;
	}
